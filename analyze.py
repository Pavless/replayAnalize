...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--battle-type", help="take only those battles with the specified battle type", type=wotReplayLib.enums.BattleType.from_string, choices=list(wotReplayLib.enums.BattleType))
    ...
    parser.add_argument("-T", "--team", help="take only those player record where the player is on the selected side", type=wotReplayLib.enums.Team.from_string, choices=list(wotReplayLib.enums.Team))
    ...
    # --- replay filters ---
    if args.incomplete:
        replay_filters.append(Filter(wotReplayLib.filters.block_count_filter, [1]))
    elif args.complete:
        replay_filters.append(Filter(wotReplayLib.filters.block_count_filter, [2]))
    elif args.unique:
        replay_filters.append(Filter(wotReplayLib.filters.block_count_filter, [2]))
        replay_filters.append(Filter(wotReplayLib.filters.unique_filter, [lambda replay: replay.id]))

    if args.map != None:
        replay_filters.append(Filter(wotReplayLib.filters.map_filter, [args.map]))
    if args.battle_type != None:
        replay_filters.append(Filter(wotReplayLib.filters.battle_type_filter, [args.battle_type]))
    if args.date != None:
        replay_filters.append(Filter(wotReplayLib.filters.date_filter, [args.date]))
    
    # --- player record filters ---
    if args.tag != None:
        player_filters.append(Filter(wotReplayLib.filters.tag_filter, [args.tag]))
    
    if args.team != None:
        player_filters.append(Filter(wotReplayLib.filters.team_filter, [args.team]))

    if args.result != None:
        player_filters.append(Filter(wotReplayLib.filters.player_battle_result_filter, [args.result]))
